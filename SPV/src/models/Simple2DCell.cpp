...
void Simple2DCell::initializeSimple2DCell(int n)
    ...
    setCellAdaptationTimeUniform(1000000000000);
...
/*sets cell adaptation time to the substrate uniformily*/
void Simple2DCell::setCellAdaptationTimeUniform(double tau)
    {
    AdaptationTime.resize(Ncells);
    ArrayHandle<double> h_tau(AdaptationTime,access_location::host,access_mode::overwrite);
    for (int ii = 0; ii < Ncells; ++ii)
        {
        h_tau.data[ii] = tau;
        };
...
/*!
sets cell adaptation time to the substrate to the input vector
*/
void Simple2DCell::setCellAdaptationTime(vector<double> &tau)
    {
    AdaptationTime.resize(Ncells);
    ArrayHandle<double> h_tau(AdaptationTime,access_location::host,access_mode::overwrite);
    for (int ii = 0; ii < Ncells; ++ii)
        {
        double save=noise.getRealUniform(0.0,1.0);
        if(save>0.5)tau[ii]=5000000;
        else tau[ii]=0.000000000001;
        h_tau.data[ii] = tau[ii];
        };
    };
