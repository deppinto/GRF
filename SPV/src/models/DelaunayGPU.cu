...
int part=33;*/
    ...
    for(jj=0; jj<poly_size; jj++)
        {
/*
if(kidx==part && prnt==true)
{
printf("edge: %i, poly: ", jj);
for(int ll=0; ll<poly_size; ll++)printf("%i, ", P_idx[baseIdx+ll]);
printf("\n");
}*/
        ...
        */
        ...
        for (int cellSpiral = 0; cellSpiral < cell_rad*cell_rad; ++cellSpiral)
            ...
            for(aa = 0; aa < d_cell_sizes[bin]; ++aa) //check points in cell
                ...
                Box.minDist(disp,currentQ,pt1); // pt1 gets overwritten by vector between new point and Pi's circumcenter
/*
if(kidx==part && newidx==566 && prnt==true)
{
printf("Found: %.20g\n",pt1.x*pt1.x+pt1.y*pt1.y-rr);
printf("rad: %.20g\n", currentRadius);
printf("vpos: %.20g, %.20g\n", v.x, v.y);
printf("otherpos: %.20g, %.20g\n", d_pt[newidx].x, d_pt[newidx].y);
printf("Qpos: %.20g, %.20g\n", currentQ.x, currentQ.y);
printf("disp: %.20g, %.20g\n", disp.x, disp.y);
printf("pt1: %.20g, %.20g, %.20g, %.20g\n", pt1.x, pt1.y, sqrt(pt1.x*pt1.x+pt1.y*pt1.y), sqrt(pt1.x*pt1.x+pt1.y*pt1.y)-currentRadius);
double2 val;
Circumcenter(P[baseIdx+jj],P[baseIdx+jj+1], val);
printf("rad: %.20g, %.20g\n", val.x, val.y);
Box.minDist(P[baseIdx+jj],currentQ,val);
printf("jj: %.20g,  %.20g\n", val.x*val.x+val.y*val.y-rr, norm(val)-currentRadius);
Box.minDist(P[baseIdx+jj+1],currentQ,val);
printf("jj+1: %.20g, %.20g\n", val.x*val.x+val.y*val.y-rr, norm(val)-currentRadius);
printf("self: %.20g\n", currentQ.x*currentQ.x+currentQ.y*currentQ.y-rr);
printf("limit: %.20g\n", eps);
}*/

                //if(pt1.x*pt1.x+pt1.y*pt1.y>rr)continue;
		if(norm(pt1)-currentRadius>eps)continue;

if(abs(norm(pt1)-currentRadius)<eps)
{
printf("Triangulation error: 4 point circumcircle\n");
d_neighnum[kidx]=0; 
return;
}
...
{
double eps=std::numeric_limits<double>::epsilon();
if(abs(pt1.x*pt1.x+pt1.y*pt1.y-rr)<eps)printf("Error\n");
else printf("ok\n");
//printf("Error: %.100g\n", pt1.x*pt1.x+pt1.y*pt1.y-rr);
}*/
...
/*if(poly_size>15){
cout<<"TOO BIG: "<<kidx<<endl;
for(int mm=0; mm<15; mm++)cout<< P_idx[mm]<< " ";
cout<<endl;
exit (911);
}*/
                        return;
...
/*if(kidx==part && prnt==true)
{
printf("final edge: %i, %i, %i, %i, %i poly: ", jj, removed, newidx, baseIdx, m);
for(int ll=0; ll<poly_size; ll++)printf("%i, ", P_idx[baseIdx+ll]);
printf("\n\n");
}*/
...
if(kidx==part && prnt==true)
{
for(int i=0; i<poly_size; i++)printf("%i, ", P_idx[baseIdx+i]);
printf("\n\n");
}*/
