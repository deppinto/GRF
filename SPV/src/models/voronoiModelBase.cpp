...
void voronoiModelBase::setSubstratePreferences(int SubIntType, double IncrementationValue, int dx, double sigmac)
    {
    SubstrateInteractions.resize(4);
    SubstrateGrid.resize(dx*dx);
    ArrayHandle<double> h_si(SubstrateInteractions,access_location::host,access_mode::overwrite);
    ArrayHandle<double> h_sg(SubstrateGrid,access_location::host,access_mode::overwrite);

    h_si.data[0]=SubIntType;
    h_si.data[1]=IncrementationValue;
    h_si.data[2]=dx;
    h_si.data[3]=sigmac;

    if(SubIntType>0)SubstrateInt=true;
    else return;

    double x11,x12,x21,x22;
    Box->getBoxDims(x11,x12,x21,x22);
    double deltax=x11/dx;
    for(int ii=0; ii<dx*dx; ii++)
        {
        double x=(ii%int(dx))*deltax;
        double y=(ii/int(dx))*deltax;
        if(x<0.5*x11)h_sg.data[ii]=-IncrementationValue;
        else h_sg.data[ii]=0;
        }
    };

//Set substrate preferences random field
void voronoiModelBase::setSubstratePreferencesRnd(int SubIntType, double mean, int dx, double sigma, int radius)
    {
    SubstrateInteractions.resize(4);
    SubstrateGrid.resize(dx*dx);
    ArrayHandle<double> h_si(SubstrateInteractions,access_location::host,access_mode::overwrite);
    ...
    cout<<"Creating substrate with mean ("<<mean<<"), sigma ("<<sigma<<"), size ("<<dx<<"), radius ("<<radius<<")"<<endl;

    if(SubIntType>0)SubstrateInt=true;
    else return;

    vector <double> sub1 (dx*dx, 0);
    for(int i=0; i<dx*dx; i++)
    {
        sub1[i]=noise.getRealNormal(0, sigma);
        if(sub1[i]>5*sigma || sub1[i]<-5*sigma)i--;
    }
...
file1.open ("sub.txt");
file1.precision(8);
file1.setf(ios::fixed | ios::showpoint);
for(int i=0; i<dx; i++){for(int j=0; j<dx; j++){int site=i+j*dx; file1<<i<<" "<<j<<" "<<h_sg.data[site]<<endl;}}*/

    };


/*!
Set the Area and Perimeter preferences according to substrate interaction
*/

void voronoiModelBase::setCellPreferencesSubstrate(double A0, double P0, bool ChangeArea, int dx)
    {
    AreaPeriPreferences.resize(Ncells);
    CellTime.resize(Ncells);
    ...
    ArrayHandle<double> h_sg(SubstrateGrid,access_location::host,access_mode::read);
    ArrayHandle<double> h_ct(CellTime,access_location::host,access_mode::overwrite);

    double x11,x12,x21,x22;
    Box->getBoxDims(x11,x12,x21,x22);
    double deltax=x11/dx;

    if(SubstrateInt==false)return;

    for (int ii = 0; ii < Ncells; ++ii)
        {
        double px = h_p.data[ii].x;
        double py = h_p.data[ii].y;
        int x=h_p.data[ii].x/deltax;
        int y=h_p.data[ii].y/deltax;
        int site = x+y*dx;
        if(ChangeArea==true)
            {
            h_ap.data[ii].x = A0+h_sg.data[site];
            h_ap.data[ii].y = P0*sqrt(h_ap.data[ii].x);
            h_ct.data[ii]=h_ap.data[ii].x;
            }
        else
            {
            h_ap.data[ii].x = A0;
            h_ap.data[ii].y = P0+h_sg.data[site];
            h_ct.data[ii]=h_ap.data[ii].y;
            }
        }
    };
...
void voronoiModelBase::movePointsSubstrate(GPUArray<double2> &displacements,double scale)
    {
    ArrayHandle<double2> d_p(cellPositions,access_location::device,access_mode::readwrite);
    ArrayHandle<double2> d_d(displacements,access_location::device,access_mode::read);
    ArrayHandle<double2> d_ap(AreaPeriPreferences,access_location::device,access_mode::readwrite);
    ArrayHandle<double> d_sg(SubstrateGrid,access_location::device,access_mode::read);
    ArrayHandle<double> d_ct(CellTime,access_location::device,access_mode::readwrite);
    ArrayHandle<double> d_tau(AdaptationTime, access_location::device,access_mode::read);    
    ArrayHandle<double> h_si(SubstrateInteractions,access_location::host,access_mode::read);

    double dx=h_si.data[2];
    double omega=h_si.data[3];
    //Dscalar tau_avg=h_si.data[3];
    
    double x11,x12,x21,x22;
    Box->getBoxDims(x11,x12,x21,x22);
    double deltax=x11/dx;
    //Dscalar Amp=tau_avg-10;
    //Dscalar tau=tau_avg;//-Amp*cos(omega*currentTime);
    
    if (scale == 1.)
      gpu_move_degrees_of_freedom_substrate(d_p.data,d_d.data,d_ap.data,
					    d_sg.data, d_ct.data, dx,
					    deltax, d_tau.data,Ncells,*(Box));
    else
      {
	cout<<"ERROR: move_points_substrate_scale" <<endl;
	exit (911);
      }
...
/*!
Displace cells on the CPU with substrate interactions
\param displacements a vector of Dscalar2 specifying how much to move every cell
\post the cells are displaced according to the input vector, and then put back in the main unit cell.
*/

void voronoiModelBase::movePointsSubstrateCPU(GPUArray<double2> &displacements,double scale)
    {
    ArrayHandle<double2> h_p(cellPositions,access_location::host,access_mode::readwrite);
    ArrayHandle<double2> h_d(displacements,access_location::host,access_mode::read);
    ArrayHandle<double2> h_ap(AreaPeriPreferences,access_location::host,access_mode::overwrite);
    ...
    ArrayHandle<double> h_sg(SubstrateGrid,access_location::host,access_mode::read);
    ArrayHandle<double> h_ct(CellTime,access_location::host,access_mode::overwrite);
    ArrayHandle<double> h_tau(AdaptationTime,access_location::host,access_mode::read);

    int subinter=h_si.data[0];
    double x11,x12,x21,x22;
    Box->getBoxDims(x11,x12,x21,x22);
    double IncVal=0;
    double dx=h_si.data[2];
    ...
    double p0;
    double omega=h_si.data[3];
    double tau_avg;//=h_si.data[3];
    double Amp;//=tau_avg-10;
    double tau;//=tau_avg;//-Amp*cos(omega*currentTime);
	...
	#pragma omp parallel for num_threads(ompThreadNum)
        for (int idx = 0; idx < Ncells; ++idx)
            {
            tau_avg=h_tau.data[idx];
            Amp=tau_avg-10;
            tau=tau_avg;//-Amp*cos(omega*currentTime);
            int x=h_p.data[idx].x/deltax;
            int y=h_p.data[idx].y/deltax;
            int site = x+y*dx;
            h_p.data[idx].x += h_d.data[idx].x;
            h_p.data[idx].y += h_d.data[idx].y;
            Box->putInBoxReal(h_p.data[idx]);
            x=h_p.data[idx].x/deltax;
            y=h_p.data[idx].y/deltax;
            int site2 = x+y*dx;
            IncVal=h_sg.data[site2]-h_sg.data[site];

            h_ct.data[idx]=h_ct.data[idx]+IncVal;
            double NextVal=h_ct.data[idx];
            double Val=0;
            ...
            switch(subinter)
                ...
                case 1:
                    Val=h_ap.data[idx].x;
                    p0=h_ap.data[idx].y/sqrt(h_ap.data[idx].x);
                    h_ap.data[idx].x = NextVal+exp(-1/tau)*(Val-NextVal);
                    h_ap.data[idx].y = p0*sqrt(h_ap.data[idx].x);
                    break;
                case 2:
                    //h_ap.data[idx].y = PrevVal+IncVal*(1-exp(-time/tau));
                    Val=h_ap.data[idx].y;
                    h_ap.data[idx].y = NextVal+exp(-1/tau)*(Val-NextVal);
                    break;
                case 3:
                    {
exit (911);
/*
                    h_mot.data[idx].x = PrevVal+IncVal*(1-exp(-time/tau));
                    Dscalar theta = h_cd.data[idx];
                    h_v.data[idx].x = h_mot.data[idx].x*cos(theta);
                    h_v.data[idx].y = h_mot.data[idx].x*sin(theta);
*/
                    }
                    break;
                case 4:
exit (911);
/*
                    h_mot.data[idx].y = PrevVal+IncVal*(1-exp(-time/tau));
*/
                    break;
                }
            };
...
void voronoiModelBase::moveDegreesOfFreedom(GPUArray<double2> &displacements,double scale)
    ...
    if (GPUcompute)
        if(SubstrateInt==false)movePoints(displacements,scale);
        else movePointsSubstrate(displacements,scale);
    else
	if(SubstrateInt==false)movePointsCPU(displacements,scale);
        else movePointsSubstrateCPU(displacements,scale);
    };
