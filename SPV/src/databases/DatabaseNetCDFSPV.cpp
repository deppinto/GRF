...
void SPVDatabaseNetCDF::SetDimVar()
    ...
    AreaPeriVar     = File.add_var("AreaPeri", ncDouble,recDim, NvDim);
    if(exclusions)
...
void SPVDatabaseNetCDF::GetDimVar()
    ...
    AreaPeriVar = File.get_var("AreaPeri");
    if(exclusions)
...
void SPVDatabaseNetCDF::WriteState(STATE s, double time, int rec)
    ...
    ArrayHandle<double2> h_ap(s->AreaPeriPreferences,access_location::host,access_mode::read);
...
//    means0 = means0/Nv;
    ...
    AreaPeriVar -> put_rec(&areaperi[0],   rec);
    if(exclusions)
        {
...
void SPVDatabaseNetCDF::ReadState(STATE t, int rec,bool geometry)
    ...
    //get the cell preferences
    AreaPeriVar-> set_cur(rec);
    std::vector<double> areaperi(Nv,0.0);
    AreaPeriVar->get(&areaperi[0],1, NvDim->size());

    ArrayHandle<double2> h_ap(t->AreaPeriPreferences,access_location::host,access_mode::overwrite);
    for (int idx = 0; idx < Nv; ++idx)
        {
	double p0=areaperi[idx];
        h_ap.data[idx].y=p0;
        };
